...
# 50197wxappJAVA的微信食堂线上订餐小程序的设计与实现ssm
...
本课题研究的微信食堂线上订餐小程序前后台分离，让商品订单，商家信息，商品信息等相关信息集中在后台让管理员管理，让用户在小程序端订餐，管理个人订单，该系统让信息管理变得高效，也让用户订餐，查看个人订单等信息变得越来越方便。
## 1.3 研究内容
本文对微信食堂线上订餐小程序的设计与实现分成六个章节来说明。

第1章：研究微信食堂线上订餐小程序的背景，以及开发微信食堂线上订餐小程序的意义。

第2章：对开发微信食堂线上订餐小程序的环境还有技术进行说明。

第3章：分析微信食堂线上订餐小程序的可行性，性能，流程以及功能。

第4章：设计微信食堂线上订餐小程序的功能结构，设计数据库E-R图以及对数据表的存储结构进行设计。

第5章：实现微信食堂线上订餐小程序的功能并进行功能界面展示。

第6章：对系统测试进行阐述，以及对本系统部分功能进行检测。
# 第2章 开发环境与技术
本章节对开发微信食堂线上订餐小程序需要搭建的开发环境，还有微信食堂线上订餐小程序开发中使用的编程技术等进行阐述。
## 2.1 Java语言  
...
### 3.1.1 技术可行性
在技术实现层次，分析了好几种技术实现方法，并且都有对应的成功案例，也有很多开源模块可以进行参考，所以从技术可行性分析来讲，实现微信食堂线上订餐小程序是没有问题的。
### 3.1.2 经济可行性
对于身为学生的开发者而言，在经济资源上面可用者很少，为了开发微信食堂线上订餐小程序，通过开发软件对硬件的要求，发现自己的电脑是完全能用来开发的，并且学校机房的配置也可以达到要求。最重要的是选择的技术都可以在网上找到不花钱的教程以及资料，因为不花钱，所以经济方面是具有可行性的。
### 3.1.3 操作可行性
微信食堂线上订餐小程序的具体实现，本身参考人类的正常操作逻辑，把常用的操作习惯当做主要的导航实现，可以让使用者更快速的理解并且上手操作，实现符合逻辑的操作流程是操作可行性的具体体现。

以上就是从不同的角度来分析，确保了微信食堂线上订餐小程序的正常开展。
## 3.2 系统流程
微信食堂线上订餐小程序投入使用后，使用者如果能看到相应的流程操作图会提高程序的理解能力。
### 3.2.1 操作流程
使用者在操作微信食堂线上订餐小程序中，应该按照本系统提供的操作流程（图3.1即为本系统的操作流程图）进行操作，可以减少操作失误，从而节省进入微信食堂线上订餐小程序的时间。

![](/md/blog.001.png)

图3.1 系统操作流程
### 3.2.2 登录流程
微信食堂线上订餐小程序通过登录功能（图3.2即为其登录的流程）引导使用者进入指定的功能操作区，也避免非本系统的用户享受本系统提供的服务以及查看本系统提供的信息，进而保证用户安全。

![](/md/blog.002.png)

图3.2 登录流程
### 3.2.3 删除信息流程
微信食堂线上订餐小程序在经过长期使用后，会产生很多的数据信息。为了腾出存储空间存放更多的数据，本系统数据库中存储的数据，一些没有参考价值的数据需要进行删除（图3.3即为删除信息的流程），删除数据过程中，为避免误删，使用者要根据系统的提示来决定是否删除数据。

![](/md/blog.003.png)

图3.3 删除信息流程
### 3.2.4 添加信息流程
微信食堂线上订餐小程序提供可视化的功能操作区，非常方便使用者进行数据操作，当使用者往系统中录入数据时（图3.4即为添加信息的流程），本系统也会进行数据合法性的判断，符合要求的数据才能够在数据库指定表中进行登记。
...
## 3.4 功能需求
微信食堂线上订餐小程序根据使用权限的角度进行功能分析，并运用用例图来展示各个权限需要操作的功能。

图3.5即为管理员用例图，管理员权限操作的功能包括对用户注册信息，对商家信息，商品信息，公告信息等进行管理。

![](/md/blog.005.png)

图3.5 管理员用例图

图3.6即为商家用例图，商家权限操作的功能包括管理商品，管理并回复商品评价，管理商品订单，查看公告等。

![](/md/blog.006.png)

图3.6 商家用例图

图3.7即为用户用例图，用户权限操作的功能包括购买商品，查看商家资料，查看公告，查看商品评价，管理商品订单，进行账户充值等。
...
图4.3 用户功能结构
## 4.3 数据库设计
微信食堂线上订餐小程序运行中产生的数据需要按照提前设置的存储规则进行保存，设计出一个符合项目的最优数据存储格式，因为它能减少用户的等待时间，还可以对系统的请求在最短时间内进行响应。所以，对数据库设计时，需要对功能需求进行详细的拆分，以及对业务状态的细分，然后设计具体的存储规则，保证数据库能正常运作，缩短数据处理时间，并在一定程度上降低数据冗余，节省存储空间。
### 4.3.1 数据库概念设计
